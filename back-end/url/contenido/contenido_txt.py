...
def download_project_content(user, repo, branch="main", output_dir="project_content"):
    ...
    print(f"📁 Descargando contenido a: {output_dir}/\n")
    ...
    for item in tree:
        ...
        if path.startswith('back-end/'):
            ...
            print(f"{Fore.YELLOW}📄 Otros:{Style.RESET_ALL} {path}")
...
def download_and_save_files(files, category, output_dir):
    """Descarga y guarda archivos de una categoría específica"""
    if not files:
        return
    ...
    print(f"\n📥 Descargando {len(files)} archivos de {category}...")
...
def main():
    ...
    try:
        ...
        print(f"   📘 Docs: {len(result['docs'])} archivos")
        print(f"   📄 Otros: {len(result['other'])} archivos")
        print(f"   📦 Total: {sum(len(v) for v in result.values())} archivos")
        print(f"\n📋 Archivos generados:")
        print(f"   📄 PROYECTO_COMPLETO.txt - Todo el código en un archivo")
        print(f"   📄 BACKEND_COMPLETO.txt  - Solo backend")
        print(f"   📄 FRONTEND_COMPLETO.txt - Solo frontend")
        print(f"   📁 {args.output}/backend/  - Archivos individuales backend")
        print(f"   📁 {args.output}/frontend/ - Archivos individuales frontend")
        print(f"   📁 {args.output}/docs/     - Archivos individuales docs")
